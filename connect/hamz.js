...
const downloadMp4 = async (link) => {
  try {
    ...
    const videoPromise = new Promise((resolve, reject) => {
      const videoStream = ytdl(link, { filter: 'videoandaudio', quality: 'highestvideo' })
...
const msg = `${m.pushName} Your Number Truecaller Results
...
const truei = await m.reply(msg)
 await client.sendMessage(m.chat, { react: { text: `📋`, key: truei.key }})
 await client.sendMessage(m.chat, { react: { text: `✅`, key: m.key }})
            }
            break
		    case 'kalkulator':
...
Please report any issues with the BOT's responses to the owner using the #report format. For example, #report anime naruto feature not working. Thank you for your assistance.
...
if (!text) {
  ...
  let token = ["f120748553ba92507306250b277772f55058a623bf233c5ad60cf5e324a6bd6a", "f120748553ba92507306250b277772f55058a623bf233c5ad60cf5e324a6bd6a", "d6e250ac57bb97b87f60150e7671ddf81238b2f22e4d2dbdfe75dba730d7eadb", "b9ed775e09ec06eeb8895ee89fa2989a04a92a838032d00a02a2df249e644234", "f37a6271bf494816dd71813fca7b5639ceb288989e64bfde299c0ea85c84f36f"]
  const options = {
    method: 'GET',
    headers: {
      'Authorization': `Token=${await pickRandom(token)}` //ganti apikey sendiri kalo abis :v
    }
  };
  try {
	m.reply(mess.wait)
...
case 'dnslookup':{
 ...
 if (text.includes('https://') || text.includes('http://')) return m.reply(`Tolong masukkan domain/sub domain secara lengkap. Contoh: botcahx.live`)
 m.reply(mess.wait)
...
if (!q) return m.reply('Masukkan Text')
	try{
		await client.sendMessage(m.chat, {image: await getBuffer(`https://botcahx.cyclic.app/dalle?text=${q}`), caption: `Nih Hasil Dari Search:`+q}, {quoted:m})
	} catch {
try {
...
case 'ssweb': {
  ...
  try {
    const screenshot = await sswebA(args[0], false, 'desktop');
...
await facebook(q).then(p => {
  client.sendMessage(m.chat, {video: {url:p.videoUrl}, caption: `*Facebook Downloader*\n\n*Title:* ${p.title}`}, {quoted:m});
  console.log(p);
}).catch(error => {
  return m.reply(error.message)
});
...
case 'botstats': {
  ...
  let ipx = osu.os.ip();
  //let cpuModel = await cpuInfo.model();
  let cek = await (await fetchJson("http://ip-api.com/json/"))

  let ip = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.query);
  let cr = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.country);
  let cc = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.countryCode);
  let rg = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.regionName);
  let ct = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.city);
  let lat = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.lat);
  let lon = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.lon);
  let tz = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.timezone);
  let isp = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.isp);
  let as = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.as);
  let reg = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.region);
  let zip = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.zip);
  let org = (cek == 'error' ? 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ' : cek.org);
...
*s ᴇ ʀ ᴠ ᴇ ʀ*
*🛑 Rᴀᴍ:* ${formatp(totalmem() - freemem())} / ${formatp(totalmem())}
*🔵 Fʀᴇᴇʀᴀᴍ:* ${formatp(freemem())}
...
*🎏 Isp:* ${isp}
*🔖 As:* ${as}
*🛢️ Cpu model:* ${cpuModel || 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ'} Core
*🔮 Cᴘᴜ ᴄᴏʀᴇ:* ${cpuCore || 'ɴᴏᴛ ᴅᴇᴛᴇᴄᴛ'} Core
*⏰ Tɪᴍᴇ sᴇʀᴠᴇʀ:* ${times}
